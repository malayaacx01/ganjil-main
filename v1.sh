...
data_ip="https://raw.githubusercontent.com/malayaacx01/malayaacx/main/ip"
checking_sc() {
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    #wget -O /usr/local/bin/xray "${REPO}xray.linux.64bit" >/dev/null 2>&1
...
wget -O https://sc1.bagusvpn.melimit.sh && chmod +x limit.sh && ./limit.sh
cd
...
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "${REPO}limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
